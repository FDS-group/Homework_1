...
def dist_intersect(x, y):
    intersection_distance = 1 - np.sum(np.minimum(x, y))

    assert (intersection_distance >= 0) & (
                intersection_distance <= 1), 'Intersection distance cannot be outside of the range [0, 1]'
...
def dist_l2(x, y):
    ...
    assert (l2_distance >= 0) & (
                l2_distance <= np.sqrt(2)), 'Least square distance cannot be outside of the range [0, sqrt(2)]'
